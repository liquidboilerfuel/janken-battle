...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (timeLeft <= 0) { handleSelect("timeout"); return; }
  ...
  const judge = (m: string, o: string) => {
    if (m === o) return 0;
    if (m === "timeout") return -1;
    if (o === "timeout") return 1;
    if ((m==="rock" && o==="scissors") || (m==="scissors" && o==="paper") || (m==="paper" && o==="rock")) return 1;
    return -1;
  };
  ...
  const handleSelect = (handName: string) => {
    if (isWaiting || myHandRef.current || !isMatched) return;
    if (handName !== "timeout") playSound('select');
    
    setIsWaiting(true);
    setMyHand(handName);
    myHandRef.current = handName;
    broadcast('move', { hand: handName });
    checkResult();
  };
  ...
  return (
    <main className="flex flex-col h-dvh bg-slate-950 text-white font-sans overflow-hidden select-none relative">
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative bg-gradient-to-b from-red-500/5 to-transparent">
        ...
        <div className={`text-[10rem] font-black tabular-nums transition-all duration-700 ${opStreak >= 3 ? 'text-red-500 scale-110' : 'text-white opacity-10'}`} style={{ filter: opStreak >= 3 ? `drop-shadow(0 0 20px rgba(239, 68, 68, 0.8))` : 'none' }}>
          {scores.op}
        </div>
        {opStreak >= 2 && <div className="absolute bottom-6 animate-bounce bg-red-600 px-3 py-1 rounded-full text-[9px] font-black italic shadow-lg">WIN STREAK: {opStreak}</div>}
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative bg-gradient-to-t from-cyan-500/5 to-transparent">
        ...
        {streak >= 2 && <div className="absolute top-12 text-cyan-400 text-[10px] font-black tracking-widest animate-pulse">🔥 {streak} WINS ON FIRE</div>}
        <div className={`text-[12rem] font-black tabular-nums transition-all ${streak >= 3 ? 'text-cyan-400' : 'text-white'}`} style={{ filter: streak >= 3 ? `drop-shadow(0 0 30px rgba(34, 211, 238, 0.6))` : 'none' }}>
          {scores.me}
        </div>
        ...
        <div className="flex w-full max-w-sm gap-4 px-8 pb-10">
          {hands.map((hand) => (
            <button
              ...
              onClick={() => handleSelect(hand.name)}
            >
